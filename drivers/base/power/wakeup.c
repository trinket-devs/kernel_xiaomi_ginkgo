...
static bool check_for_block(struct wakeup_source *ws)
{
	...
	//1.0.1 only if ws structure is valid
	if (ws)
	{
		// wake lock names handled have maximum length=50 and minimum=1 1.1.0
		...
		// 1.0.1 if it is currently active, deactivate it immediately + log in debug mode
		if (ws->active)
		{
